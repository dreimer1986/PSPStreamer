...
static unsigned long long input_until,input_next;
...
static void input_clear_keys(void) {
    input_buttons=0;input_x=input_y=128;input_until=0;
}
...
static void input_receive(unsigned long long now) {
    ...
    if(sscanf(input_reply,"%u %u %u %u %u %u %u %512s",&seq,&kind,&mask,&x,&y,&ttl,&field,hex)!=8 ||
       seq>0x7fffffff || kind>2 || (mask&~0xF3F9u) || x>255 || y>255 || ttl>1000) {
    ...
    } else if(kind==0 || seq>(unsigned)input_ack) {
        input_buttons=mask;input_x=x;input_y=y;input_until=now+ttl*1000ULL;
    }
...
static void input_remote_tick(SceCtrlData *pad) {
    ...
    if(now>=input_until)input_clear_keys();
