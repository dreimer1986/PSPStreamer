...
static int scenario,server_https,server_port=80,have_cached_server_address=1;
...
static int sceNetInetRecv(int a,void *b,int c,int d){
    ...
    const char *reply=scenario==3?"HTTP/1.0 401 Denied\r\nContent-Length: 0\r\n\r\n":
...
int main(void){
    ...
    for(scenario=0;scenario<=6;scenario++) {
        tick=0;server_https=scenario==6;
