...
(async()=>{
  ...
  try{
    ...
    let dlna={enabled:false,devices:[]};
    await page.route('http://psp.test/**',async route=>{
      ...
      if(url.pathname.startsWith('/api/')){
        ...
        switch(url.pathname){
          ...
          case '/api/dlna':data=dlna;break;
          case '/api/dlna/add':dlna={enabled:true,devices:[{id:'test',name:'Test NAS',url:body.url}]};data=dlna;break;
          case '/api/dlna/discover':data={...dlna,discovered:1};break;
          case '/api/dlna/settings':if(body.remove)dlna.devices=[];if('enabled' in body)dlna.enabled=body.enabled;data=dlna;break;
    ...
    await page.fill('#dlnaUrl','http://192.168.1.10:8200/rootDesc.xml');
    await page.locator('#dlnaForm button').click();
    await page.waitForFunction(()=>document.querySelector('#sourceDlna').checked);
    await page.getByText('Test NAS — http://192.168.1.10:8200/rootDesc.xml',{exact:true}).waitFor();
    await click('#dlnaDiscover');
    await page.waitForFunction(()=>document.querySelector('#dlnaStatus').textContent.includes('1'));
    await page.locator('#dlnaDevices button').click();
    await page.waitForFunction(()=>!document.querySelector('#dlnaDevices').children.length);
