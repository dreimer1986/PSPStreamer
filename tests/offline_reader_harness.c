...
static int video_pts=-1,audio_pts=-1,frames,samples;
...
static int sceIoOpen(const char *path,int flags,int mode){(void)flags;(void)mode;source=fopen(path,"rb");return source?1:-1;}
static int sceIoRead(int fd,void *out,int size){assert(fd==1);if(size>7)size=7;return fread(out,1,size,source);}
