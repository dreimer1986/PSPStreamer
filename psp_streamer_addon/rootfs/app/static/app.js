...
async function browse(next=''){
  ...
  $('#batchTools').hidden=['',':files:',':plex:',':jellyfin:',':dlna:',':jellyfin:playlists',':radio:'].includes(path);
...
async function dlnaRefresh(){const s=await api('/api/dlna');$('#sourceDlna').checked=s.enabled;$('#dlnaDevices').replaceChildren();for(const device of s.devices){const row=document.createElement('p');const label=document.createElement('span');label.textContent=device.name+' — '+device.url;row.append(label,button(t('Remove'),async()=>{await post('/api/dlna/settings',{remove:device.id});await dlnaRefresh();await browse('');}));$('#dlnaDevices').append(row);}}
action('#dlnaDiscover',async()=>{$('#dlnaStatus').textContent=t('Searching local network…');try{const s=await post('/api/dlna/discover');$('#dlnaStatus').textContent=t('DLNA servers found: {count}',{count:s.discovered});await dlnaRefresh();await browse('');}catch(e){$('#dlnaStatus').textContent=e.message;throw e;}});
$('#dlnaForm').onsubmit=async event=>{event.preventDefault();try{await post('/api/dlna/add',{url:$('#dlnaUrl').value});await dlnaRefresh();await browse('');message(t('Saved. Refresh the PSP library with Square.'));}catch(e){fail(e)}};
$('#sourceForm').onsubmit=async event=>{event.preventDefault();try{const jf=$('#sourceJellyfin').checked,dlna=$('#sourceDlna').checked;if(!jf&&!dlna&&!$('#sourceFiles').checked&&!$('#sourcePlex').checked&&!$('#sourceRadio').checked)throw Error(t('Keep at least one source enabled'));const mappings=[...document.querySelectorAll('.mapping')].map(row=>{const fields=row.querySelectorAll('input');return {plex:fields[0].value,local:fields[1].value};});if(dlna)await post('/api/dlna/settings',{enabled:true});if(jf)await post('/api/jellyfin/settings',{enabled:true});await post('/api/plex/settings',{files:$('#sourceFiles').checked,enabled:$('#sourcePlex').checked,radio:$('#sourceRadio').checked,mappings});if(!jf)await post('/api/jellyfin/settings',{enabled:false});if(!dlna)await post('/api/dlna/settings',{enabled:false});await Promise.all([plexRefresh(),jellyfinRefresh(),dlnaRefresh()]);await browse('');message(t('Saved. Refresh the PSP library with Square.'));}catch(e){fail(e)}};
