...
let providerPage=0,providerGeneration=0;
const providerLoad=button(t('Refresh'),()=>loadProvider(0));
providerPanel.append(providerHeading,providerSelect,providerShelf,providerSection,providerLoad,providerRows,providerPages);$('#view-library').after(providerPanel);
...
async function loadProviderSections(){
 const generation=++providerGeneration;providerRows.textContent=t('Loading tracks…');
 try{
...
async function loadProvider(offset=0){
 const generation=++providerGeneration;providerPage=offset;providerRows.textContent=t('Loading tracks…');providerPages.replaceChildren();
